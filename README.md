...
- **Build** — Two sibling sub-modes that mirror SERIS's two submission styles:
  - **OR Case · Messages** — Assemble the **complete** SERIS message Bundle for a case event
    ...
    MessageHeader, Task, Patient, Practitioner, PractitionerRole, Appointment, Encounter, Procedure
  ...
  - **OR Schedule · REST creates** — The schedule side isn't a message: per the client
    ...
    `POST /Slot` (bookable) → `POST /Slot` (block) — each a discrete request that references the
    earlier ones by business identifier, every resource conformant. (The OR Schedule track in
    Orientation links straight here.)
